                ...
                <main role="main" style="margin-left: 30px;">
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
                        ...
                        <div class="btn-toolbar mb-2 mb-md-0">
                            <div class="btn-group mr-2">
                                <a href="{{ route('vacancy.admin') }}"><button type="button"
                                        class="btn btn-sm btn-outline-secondary dropdown-toggle">
                                        <span data-feather="calendar"></span>
                                        {{ __('Cписок') }}
                                    </button></a>
                                <a href="{{ route('vacancy.create') }}"><button type="button"
                                        class="btn btn-sm btn-outline-secondary">{{ __('Добавить вакансию') }}
                    ...
                    <div class="table-responsive"  id="project-container">
                        <table class="table table-striped table-sm">
                            ...
                            <tbody>
                                @if ($news->isEmpty())
                                    <p>{{__('Вакансии не найдены')}}</p>
                                @else
                                    @foreach ($news as $newsItem)
                                        <tr>
                                            <td>{{ $newsItem->id }}</td>
                                            <td>{{ $newsItem->vacancy }}</td>
                                            <td>
                                                <div class="text-cell" id="duties-{{ $newsItem->id }}">{{ $newsItem->duties }}
                                                </div>
                                                <span class="expand-btn"
                                                    onclick="toggleContent('duties-{{ $newsItem->id }}', this)">{{__('Показать
                                                    больше')}}</span>
                                            </td>
                                            <td>
                                                <div class="text-cell" id="requirements-{{ $newsItem->id }}">{{ $newsItem->requirements }}
                                                </div>
                                                <span class="expand-btn"
                                                    onclick="toggleContent('requirements-{{ $newsItem->id }}', this)">{{__('Показать
                                                    больше')}}</span>
                                            </td>
                                            <td>
                                                <div class="text-cell" id="conditions-{{ $newsItem->id }}">{{ $newsItem->conditions }}
                                                </div>
                                                <span class="expand-btn"
                                                    onclick="toggleContent('conditions-{{ $newsItem->id }}', this)">{{__('Показать
                                                    больше')}}</span>
                                            </td>
                                            <td>
                                            ...
                                            <td style="min-width: 160px">
                                                <div class="btn-group-line">
                                                    ...
                                                    @if ($newsItem->active === 0)
                                                        <form class="pb-2" action="{{ route('vacancy.updateActive', $newsItem->id) }}"
                                                              method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                    ...
                                                    <form action="{{ route('vacancy.show', $newsItem->id) }}" method="get">
                                                        <button type="submit" class="btn btn-sm btn-outline-secondary">
                                                            {{ __('Смотреть') }}
                                                    ...
                                                    <form class="py-2" action="{{ route('vacancy.edit', $newsItem->id) }}" method="get">
                                                        <button type="submit" class="btn btn-sm btn-outline-secondary">
                                                            {{ __('Редактировать') }}
                                                    ...
                                                    <form action="{{ route('vacancy.destroy', $newsItem->id) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('DELETE')
