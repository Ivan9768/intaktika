            ...
            <div class="col-10">
                <main role="main" style="margin-left: 30px;">
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
                        <h1 class="h2">{{ __('Новости') }}</h1>
                        <div class="row">

                            <div class="form-check-inline">
                                <form class="form-check-inline" action="{{ route('news.admin') }}" method="GET">
                                    <button style=" font-size: 14px;" class="btn btn-sm btn-outline-secondary" type="submit">
                                        {{__('Старые')}}
                                        <i class="bi bi-sort-numeric-down"></i>
                                    </button>
                                </form>
                                <form class="form-check-inline" action="{{ route('news.dateSearch') }}" method="GET">
                                    <button style="font-size: 14px;" class="btn btn-sm btn-outline-secondary" type="submit">
                                        {{__('Новые')}}
                        ...
                        <div class="btn-toolbar mb-2 mb-md-0">
                            <div class="btn-group mr-2">
                                <a href="{{ route('news.admin') }}"><button type="button"
                                        class="btn btn-sm btn-outline-secondary dropdown-toggle">
                                        <span data-feather="calendar"></span>
                                        {{ __('Cписок') }}
                                    </button></a>
                                <a href="{{ route('news.create') }}"><button type="button"
                                        class="btn btn-sm btn-outline-secondary">{{ __('Добавить новость') }}
                            ...
                            <tbody>
                                ...
                                @else
                                    @foreach ($news as $newsItem)
                                        <tr>
                                            <td>{{ $newsItem->id }}</td>
                                            <td>{{ $newsItem->datetime_publication }}</td>
                                            ...
                                            <td>
                                                <div class="text-cell" id="info-{{ $newsItem->id }}">{{ $newsItem->info }}
                                                </div>
                                                <span class="expand-btn"
                                                    onclick="toggleContent('info-{{ $newsItem->id }}', this)">Показать
                                                    больше</span>
                                            </td>
                                            <td>
                                            ...
                                            <td style="min-width: 160px">
                                                <div class="btn-group-line">
                                                    ...
                                                    @if ($newsItem->active === 0)
                                                        <form class="pb-2" action="{{ route('news.updateActive', $newsItem->id) }}"
                                                              method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                    ...
                                                    <form  action="{{ route('news.show', $newsItem->id) }}" method="get">
                                                        <button type="submit" class="btn btn-sm btn-outline-secondary">
                                                            {{ __('Смотреть') }}
                                                    ...
                                                    <form class="py-2" action="{{ route('news.edit', $newsItem->id) }}" method="get">
                                                        <button type="submit" class="btn btn-sm btn-outline-secondary">
                                                            {{ __('Редактировать') }}
